...
/**
 *  Fetches all question papers within an Institution and includes:
 *  - Institution details in `belongsTo`
 *  - Creator details in `createdBy`
 *  - A signed URL for accessing the document
 */
export const getQuestionPapersByInstitution = async (req, res) => {
  try {
    const { institution_id } = req.params;
    const querySnapshot = await db.collection("questionPapers").where("belongsTo", "==", institution_id).get();
    ...
    const questionPapers = await Promise.all(
      querySnapshot.docs.map(async (doc) => {
        ...
        let institutionData = { inst_id: institutionId, name: null };
